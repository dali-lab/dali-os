...
import { loader as pageLoader } from "~/admin-console/routes/admin-console.payroll";
import { loader as csvLoader } from "~/admin-console/routes/admin-console.payroll.csv";
...
describe("payroll page loader — auth gate", () => {
  it("redirects anonymous users to /login", async () => {
    ...
    const res = (await pageLoader({
      request: req("http://localhost/admin-console/payroll"),
      params: {},
      context: {},
    } as any)) as Response;
  ...
  it("redirects non-admins to /admin-console/members", async () => {
    asNonAdmin();
    const res = (await pageLoader({
      request: req("http://localhost/admin-console/payroll"),
      params: {},
  ...
  it("loads reconciliation for an admin", async () => {
    ...
    const data = await pageLoader({
      request: req("http://localhost/admin-console/payroll?term=term-25f"),
      params: {},
      context: {},
    } as any);
...
describe("payroll.csv loader — auth gate + view", () => {
  it("redirects anonymous users to /login", async () => {
    ...
    const res = (await csvLoader({
      request: req("http://localhost/admin-console/payroll.csv?view=summary"),
      params: {},
      context: {},
    } as any)) as Response;
    expect(res.status).toBe(302);
  ...
  it("rejects an unknown view with 400", async () => {
    asAdmin();
    const res = (await csvLoader({
      request: req("http://localhost/admin-console/payroll.csv?view=bogus"),
      params: {},
      context: {},
    } as any)) as Response;
  ...
  it("streams a CSV for a valid view as an admin", async () => {
    asAdmin();
    const res = (await csvLoader({
      request: req("http://localhost/admin-console/payroll.csv?view=summary&term=term-25f"),
      params: {},
      context: {},
    } as any)) as Response;
