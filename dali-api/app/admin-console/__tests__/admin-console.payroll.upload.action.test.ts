...
vi.mock("~/admin-console/lib/payroll-ingest.server", () => ({
...
import { action } from "~/admin-console/routes/admin-console.payroll.upload";
...
function uploadRequest(files: { timesheet?: string; notes?: string }) {
  ...
  return new Request("http://localhost/admin-console/payroll/upload", {
    method: "POST",
    body: fd,
  });
}
