...
describe("auto-assign action without challenges", () => {
  it("skips the per-domain rubric requirement without challenges", async () => {
  ...
  it("still requires the cycle-level general rubric without challenges", async () => {
    mockPrisma.applicationCycle.findUniqueOrThrow.mockResolvedValueOnce({
      id: CYCLE_ID,
  ...
  it("filters DomainApplications by direct domainId (works with no challengeVersion)", async () => {
    await callAction();
