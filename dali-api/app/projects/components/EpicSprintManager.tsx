...
// One row of the design's add-menu: glyph, white label, container fill on
// hover. Each item carries its own icon so the New menu reads at a glance
// (a task check, a story doc, an epic bolt, a meeting calendar).
function AddMenuItem({
  ...
  return (
    ...
    >
      <span className="flex h-4 w-4 flex-shrink-0 items-center justify-center text-os-accent">
...
export function EpicSprintManager({
  ...
  // The Progress people filter narrows the timeline to the chosen people: a
  // task survives if one of its assignees is selected, a story if it keeps a
  // task, an epic if it keeps a story. Nobody selected = timeline untouched.
  const filteredTimelineEpics = useMemo(() => {
    if (selectedPeopleIds.length === 0) return timelineEpics;
    const want = new Set(selectedPeopleIds);
    return timelineEpics
      .map((e) => ({
        ...
        stories: e.stories
          .map((s) => ({
            ...s,
            tasks: s.tasks.filter((t) => t.assignees.some((a) => want.has(a.id))),
          }))
          .filter((s) => s.tasks.length > 0),
      }))
      .filter((e) => e.stories.length > 0);
  }, [timelineEpics, selectedPeopleIds]);

  // The Progress toolbar: the people filter on the left, Edit + New on the
  // right, sitting above the timeline. Shown under os; Edit/New need manage
  // rights, the filter only needs someone to filter by.
  const showToolbar = os && (canManage || peopleOptions.length > 0);
  const progressToolbar = showToolbar ? (
    <div className="flex items-center justify-between gap-2.5">
      {peopleOptions.length > 0 && onPeopleChange ? (
        <PeopleFilter
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* The design's toolbar sits above the timeline: people filter left,
          Edit + New right. Its Add menu opens downward into the timeline. */}
      {progressToolbar}
    ...
    <div className="max-h-[80vh] overflow-y-auto">
      ...
      <div className="mb-6 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            {/* The level is named with a badge, not marked with a dot. */}
            <span className="os-type-badge os-type-badge--epic flex-shrink-0">Epic</span>
            {canManage ? (
              <input
                ...
                className={cn(
                  "os-record-name w-full font-heading text-lg font-bold text-foreground bg-transparent rounded px-1 -mx-1 py-0.5 hover:bg-muted/40 focus:bg-transparent focus:outline-none focus:ring-2 focus:ring-accent-coral/30",
                )}
              />
            ) : (
              <h2
                id="epic-detail-title"
                className="os-record-name font-heading text-lg font-bold text-foreground truncate"
              >
                {epic.title}
              </h2>
            )}
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          {/* A section is named in type, not with a colour dot. */}
          <h3 className="os-section-header !mb-0 flex items-center gap-2">
            User stories
          </h3>
