...
import type { Route } from "./+types/education.$offeringId.setup";
...
export const meta: Route.MetaFunction = ({ data }) => [
  { title: `Setup · ${data?.offering?.title ?? "Course"} · DALI OS` },
];
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  if (!redesign) {
    return redirect(`/education/manage/${params.offeringId}`);
  }
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  if (
    ...
    return redirect(`/education/manage/${(result as { id: string }).id}`);
  return result;
}
