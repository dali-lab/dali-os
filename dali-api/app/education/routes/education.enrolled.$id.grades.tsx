...
import type { Route } from "./+types/education.enrolled.$id.grades";
import { requireEnrollment } from "~/education/lib/auth";
import { prisma } from "~/lib/db";

export async function loader({ request, params }: Route.LoaderArgs) {
  const { user, isManager } = await requireEnrollment(request, params.id);
  ...
  // Managers previewing don't have submissions of their own.
  const submissions = isManager
    ? []
...
export default function EnrolledGrades() {
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <Link
                    to={`/education/enrolled/${offeringId}/assignments/${g.assignmentId}`}
