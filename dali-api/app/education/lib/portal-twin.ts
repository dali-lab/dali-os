...
//                                             → /portal/education/…
// Returns null for /education/manage (external instructors stay in the shell)
// and for /education/compliance (Core-only) and /education/check-in/:id (no
// portal route) — the caller decides stay-vs-/portal for those.
//
...
export function educationPortalTwin(pathname: string): string | null {
  ...
  const offeringTwin =
    /^\/education\/(?!compliance$|check-in\/)[^/]+(?:\/(?:apply|hub|page\/[^/]+|assignments\/[^/]+))?$/;
