...
export const MANAGE_PARTNER_APPLICATION_TOOL = {
  ...
  description:
    "Create and manage partner applications. Actions: create (title+partnerOrgId required), update_status (applicationId+status required), update_details (applicationId required; updates summary/terms), set_form (formId required; replaces the global application form binding), clear_form (removes the form binding), add_domain (applicationId+domainId required), update_domain_scope (applicationDomainId required; updates expectedMembers), remove_domain (applicationDomainId required). Requires Core access.",
  inputSchema: {
    ...
    properties: {
      ...
      title: { type: "string" },
      partnerOrgId: { type: "string", description: "Required for create." },
      status: {
        type: "string",
...
export async function runManagePartnerApplication(
  ...
  if (action === "create") {
    const partnerOrg = await prisma.partnerOrg.findUnique({
      where: { id: input.partnerOrgId as string },
      select: { id: true },
    });
    if (!partnerOrg) {
      throw new McpNotFoundError(`Partner organization ${input.partnerOrgId} not found`);
    }
    const app = await prisma.partnerApplication.create({
      data: {
        title: (input.title as string).trim(),
        partnerOrgId: input.partnerOrgId as string,
      },
      select: { id: true },
    });
    return { id: app.id };
