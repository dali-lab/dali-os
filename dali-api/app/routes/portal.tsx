...
function InvitedToInterviewView({
  ...
  useEffect(() => {
    ...
    fetch(`/api/cycles/${cycleId}/available-slots?domainId=${domainApp.domainId}&mode=${mode}`, {
      credentials: "include",
    })
      .then(r => r.ok ? r.json() : [])
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const slotsRes = await fetch(`/api/cycles/${cycleId}/available-slots?domainId=${domainApp.domainId}&mode=${mode}`, { credentials: "include" });
        if (slotsRes.ok) {
...
function InterviewScheduledView({
  ...
  useEffect(() => {
    ...
    setRescheduleSlots([]);
    fetch(`/api/cycles/${cycleId}/available-slots?domainId=${domainApp.domainId}&mode=${rescheduleMode}`, { credentials: "include" })
      .then(r => r.ok ? r.json() : [])
      .then((apiSlots: { startTime: string; endTime: string }[]) => {
        setRescheduleSlots(
