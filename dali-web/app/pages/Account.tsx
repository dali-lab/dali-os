...
export default function Account() {
  ...
  useEffect(() => {
    ...
    // Fetch application open status + form questions in parallel (public endpoints)
    Promise.all([
      fetch(`${DALI_DB_URL}/application-status`).then(r => r.json()).catch(() => null),
      fetch(`${DALI_DB_URL}/form-questions`).then(r => r.json()).catch(() => ({ common: [], domains: {} })),
    ]).then(([status, questions]) => {
      if (status) setAppOpenStatus(status);
    ...
    if (s.type === 'dartmouth' || s.type === 'member') {
      fetch(`${DALI_DB_URL}/users/${s.account.id}`, {
        ...
        .then(data => {
          ...
          // Auto-open draft if one exists for the current term
          const draft = apps.find(a => a.status === 'IN_PROGRESS');
